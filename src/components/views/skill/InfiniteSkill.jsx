...
const InfiniteSkill = ({skills}) => {
    ...
    if (skills && skills.length > 0) {
       ...
       const Backend = Array.isArray(skills) ? skills.filter((item) => item.type === 'backend') : [];
const Frontend = Array.isArray(skills) ? skills.filter((item) => item.type === 'frontend') : [];
const Utilities = Array.isArray(skills) ? skills.filter((item) => item.type === 'utility') : [];
